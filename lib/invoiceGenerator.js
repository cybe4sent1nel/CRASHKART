 ...
 */
// Lightweight entry that returns HTML in non-production and delegates to a
// production-only module when running in production. This file intentionally
// avoids any static reference to `pdfkit` or `fontkit` so Turbopack won't
// include those modules in development builds.
export async function getInvoiceAttachment(orderData) {
    if (process.env.NODE_ENV !== 'production') {
        try {
            const htmlContent = generateInvoiceHTML(orderData)
            return { filename: `Invoice-${orderData.orderId}.html`, content: htmlContent, contentType: 'text/html' }
        } catch (err) {
            console.error('Failed to generate HTML invoice:', err)
            return null
        }
    }

    // Production: prefer Puppeteer PDF (no pdfkit/fontkit to avoid SWC issues), fall back to HTML.
    try {
        const { generateInvoicePdf } = await import('./invoicePuppeteer.js')
        ...
        console.warn('Puppeteer invoice generation failed in production, falling back to HTML:', err?.message || err)
    }
    ...
    try {
        ...
        console.error('Failed to generate production invoice fallback HTML:', err)
        return null
    }
}
