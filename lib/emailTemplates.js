...
// Helper function for getting logo URL
const getLogoUrl = () => {
    const baseUrl = process.env.NEXT_PUBLIC_APP_URL || 'https://crashkart.vercel.app';
    return `${baseUrl}/LOGO.png`;
};

// Helper function for logo header with hosted URL (works in all environments)
const getLogoHeader = (useLogo = true) => {
    return `
        <div style="background-color: #f3f4f6; padding: 20px; text-align: center; border-radius: 8px; margin-bottom: 20px;">
            ${useLogo ? `<img src="${getLogoUrl()}" alt="CrashKart" style="max-width: 150px; height: auto; margin-bottom: 10px; display: block; margin-left: auto; margin-right: auto;">` : ''}
