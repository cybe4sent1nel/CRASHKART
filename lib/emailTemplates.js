...
// Helper to safely get customer name with fallbacks
...
export const orderConfirmationEmail = (order, trackingLink) => {
    ...
    const safeName = getSafeName(customerName);

    const shipping = deliveryBreakdown.shipping || {};
    const convenience = deliveryBreakdown.convenience || {};
    const platform = deliveryBreakdown.platform || {};
    const feeWaiverNote = (fee) => (fee.amount === 0 && Number(fee.base || 0) > 0
        ? ` (waived${fee.reason ? ` — ${fee.reason}` : ''})`
        : '');

    const shippingLine = `${currency}${Number(shipping.amount || 0).toFixed(2)}${feeWaiverNote(shipping)}`;
    const convenienceLine = `${currency}${Number(convenience.amount || 0).toFixed(2)}${feeWaiverNote(convenience)}`;
    const platformLine = `${currency}${Number(platform.amount || 0).toFixed(2)}${feeWaiverNote(platform)}`;

    const couponDiscountValue = typeof couponDiscount === 'number' ? couponDiscount : (typeof discount === 'number' ? discount : 0);
    const crashCashAppliedValue = Number(crashCashApplied || 0);
    const deliveryTotal = typeof deliveryCharge === 'number'
        ? Number(deliveryCharge || 0)
        : Number(shipping.amount || 0) + Number(convenience.amount || 0) + Number(platform.amount || 0);
                    ...
                    <div class="header">
                     ...
                     <div class="content">
                        ...
                        <div class="section">
                            ...
                            <div class="summary-row">
                                <span>Shipping Fee${shipping.reason ? ` <span class="badge">${shipping.reason}</span>` : ''}</span>
                                <span>${shippingLine}</span>
                            </div>
                            <div class="summary-row">
                                <span>Convenience Fee${convenience.reason ? ` <span class="badge">${convenience.reason}</span>` : ''}</span>
                                <span>${convenienceLine}</span>
                            </div>
                            <div class="summary-row">
                                <span>Platform Fee${platform.reason ? ` <span class="badge">${platform.reason}</span>` : ''}</span>
                                <span>${platformLine}</span>
                            </div>
                            ${deliveryTotal ? `
                                <div class="summary-row">
                                    <span>Delivery Charges Total:</span>
                                    <span>${currency}${deliveryTotal.toFixed(2)}</span>
                                </div>
                            ` : ''}
                            ${couponDiscountValue ? `
                                <div class="summary-row">
                                    <span>Coupon Savings:</span>
                                    <span style="color: #22c55e;">-${currency}${couponDiscountValue.toFixed(2)}</span>
                                </div>
                            ` : ''}
                            ${crashCashAppliedValue ? `
                                <div class="summary-row">
                                    <span>CrashCash Applied:</span>
                                    <span style="color: #22c55e;">-${currency}${crashCashAppliedValue.toFixed(2)}</span>
                                </div>
                            ` : ''}
...
export const paymentReceivedEmail = (name, order) => {
    const safeName = getSafeName(name);
    const { orderId, items = [], total, paymentMethod } = order || {};
        ...
        html: `
            ...
            <head>
                ...
                <style>
                    ...
                    .order-id { background: #f3f4f6; padding: 15px; border-radius: 8px; margin: 20px 0; text-align: center; }
                    .order-id .label { color: #6b7280; font-size: 14px; }
                    .order-id .value { font-size: 24px; font-weight: bold; color: #22c55e; }
                    .section { margin: 30px 0; }
                    .section-title { font-size: 18px; font-weight: 600; margin-bottom: 15px; color: #1f2937; border-bottom: 2px solid #22c55e; padding-bottom: 10px; }
                    table { width: 100%; border-collapse: collapse; }
                    .items-table { margin: 20px 0; }
                    .summary-row { display: flex; justify-content: space-between; padding: 10px 0; }
                    .summary-row.total { background: #f3f4f6; padding: 15px; border-radius: 8px; font-size: 18px; font-weight: bold; color: #1f2937; margin-top: 15px; }
                    .info-box { background: #f0fdf4; border-left: 4px solid #22c55e; padding: 15px; border-radius: 4px; margin: 20px 0; }
                    .footer { background: #f9fafb; padding: 20px; text-align: center; color: #6b7280; font-size: 14px; }
            ...
            <body>
                <div class="container">
                    ...
                    <div class="header">
                     ...
                     <div class="content">
                         <p>Hi <strong>${safeName}</strong>,</p>
                        <p>We have successfully received your payment for order <strong>${orderId}</strong>. We'll process your order and send a confirmation email once the order is fully confirmed.</p>
                        ...
                        <div class="section">
                            <div class="section-title">💳 Payment Method</div>
                            <p><strong>${paymentMethod || 'COD'}</strong> <span class="badge">CONFIRMED</span></p>
                        </div>
                        ...
                        <p style="margin-top: 30px; padding-top: 20px; border-top: 1px solid #e5e7eb;">
                            If you have any questions, please <a href="mailto:crashkart.help@gmail.com" style="color: #22c55e; text-decoration: none;">contact our support team</a>.
                        </p>
