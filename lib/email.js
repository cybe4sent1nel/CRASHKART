...
function getLogoAttachment() {
  try {
    const logoPath = path.join(__dirname, '../public/logo.bmp');
    
    // Check if logo file exists before returning
    if (fs.existsSync(logoPath)) {
      return {
        filename: 'crashkart-logo.bmp',
        path: logoPath,
        cid: 'crashkart-logo'
      };
    } else {
    ...
    console.warn('Logo file error:', error.message);
    return null;
  }
}
...
async function sendEmailInternal(to, subject, html, fromName = APP_NAME, attachments = null) {
  try {
    ...
    if (attachments === 'include-logo') {
      ...
      if (logoAttachment) {
        // Verify file exists before adding to attachments
        if (fs.existsSync(logoAttachment.path)) {
          attachmentList.push(logoAttachment);
        } else {
          console.warn('Logo file not found, skipping attachment:', logoAttachment.path);
        }
      }
    } else if (attachments === 'include-logo-and-products') {
      // This is handled separately in the order email functions
      const logoAttachment = getLogoAttachment();
    ...
    <body>
      <div class="email-wrapper">
        <div class="logo-header">
          <img src="cid:crashkart-logo" alt="CrashKart" />
          <h1>crashkart</h1>
 ...
 */
export async function sendOrderConfirmationWithInvoice(email, orderData, invoiceBuffer, name = 'Customer') {
   const template = emailTemplates.orderConfirmationEmail(orderData, `${process.env.NEXT_PUBLIC_APP_URL}/my-orders/${orderData.orderId}`);
