...
import { sendOrderConfirmationEmail, sendOrderShippedEmail, sendOrderDeliveredEmail, sendOrderStatusEmail, sendPromotionalNewsletterEmail } from './email.js';
...
export async function triggerOrderConfirmationEmail(order, user, address, orderData) {
    try {
        ...
        const emailData = {
            ...
            trackingLink: `${process.env.NEXT_PUBLIC_APP_URL || 'http://localhost:3000'}/track/${order.id}`
        };
