...
export default function CrashCashBalance() {
    ...
    useEffect(() => {
        const loadData = () => {
            ...
            // Calculate total balance from actual rewards
            let calculatedBalance = 0
            
            // Add crashcash from scratch cards
            scratchRewardsOnly.forEach(reward => {
                if (reward.rewardType === 'crashcash' && reward.crashcash) {
                    calculatedBalance += reward.crashcash
                } else if (reward.rewardType === 'discount') {
                    // For discount rewards, add the bonus crashcash if any
                    calculatedBalance += reward.bonusCrashcash || 0
                }
            })
            
            // Add crashcash from orders
            orders.forEach(order => {
                if (order.amount) {
                    calculatedBalance += order.amount
                }
            })
            ...
            // Also update localStorage with correct balance
            localStorage.setItem('crashcashBalance', JSON.stringify(calculatedBalance))
