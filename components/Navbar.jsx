...
import { getUserCrashCash, migrateOldCrashCash } from "@/lib/userCrashcashUtils";
...
export default function Navbar() {
  ...
  useEffect(() => {
    ...
    // Load crash cash for the logged-in user
    const loadCrashCash = (userData) => {
      if (userData) {
        try {
          const email = userData.email;
          if (email) {
            const userCashData = getUserCrashCash(email);
            setCrashCash(userCashData.balance || 0);
          } else {
            setCrashCash(0);
          }
        } catch (e) {
    ...
    loadCrashCash(userData);
    
    // Migrate and initialize user data on first load
    if (userData && userData.email) {
      try {
        migrateOldCrashCash(userData.email);
    ...
    const handleCrashcashUpdate = () => {
      ...
      if (currentUserData) {
        loadCrashCash(JSON.parse(currentUserData));
      }
    };
