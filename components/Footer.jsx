...
const Footer = () => {
    ...
    const GithubIcon = () => (<svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg"> <path d="M10 1.66667C5.40833 1.66667 1.66667 5.40833 1.66667 10C1.66667 13.6817 3.93333 16.7933 7.06667 17.9517C7.48333 18.025 7.625 17.7583 7.625 17.525V16.0067C5.46667 16.4783 5 14.8633 5 14.8633C4.62 13.9 3.97 13.6367 3.97 13.6367C3.0867 13.0583 4.05 13.0717 4.05 13.0717C5.04167 13.1417 5.5 14.0067 5.5 14.0067C6.36667 15.3933 7.75833 14.91 8.27333 14.6767C8.34167 14.075 8.59167 13.6633 8.86667 13.4083C7.12833 13.1483 5.27 12.5167 5.27 9.69167C5.27 8.85833 5.55333 8.18333 6.51667 7.67C6.43333 7.41167 6.13333 6.65 6.6 5.63333C6.6 5.63333 7.45333 5.39833 8.85333 6.52333C9.64167 6.29833 10.4867 6.18333 11.325 6.18333C12.1633 6.18333 13.0083 6.30167 13.8 6.53C15.1967 5.40167 16.05 5.64 16.05 5.64C16.5083 6.66333 16.2083 7.42333 16.125 7.68333C17.0917 8.19167 17.3733 8.87 17.3733 9.70333C17.3733 12.5283 15.5117 13.1483 13.7667 13.4017C14.1 13.7083 14.3917 14.3167 14.3917 15.2517V17.525C14.3917 17.7683 14.5333 18.0417 14.9617 17.9433C18.0883 16.7817 20.3333 13.675 20.3333 10C20.3333 5.40833 16.5917 1.66667 10 1.66667Z" fill="#90A1B9"/> </svg>)
    ...
    const linkSections = [
        ...
        },
        {
            title: "CONTACT",
            links: [
                { text: "1-800-CRASH-KART", path: 'tel:1-800-2727-5278', icon: PhoneIcon },
                { text: "crashkart.help@gmail.com", path: 'mailto:crashkart.help@gmail.com', icon: MailIcon },
                { text: "Feedback: crashkart.help@gmail.com", path: 'mailto:crashkart.help@gmail.com', icon: MailIcon }
            ]
        }
    ];
    ...
    const socialIcons = [
        ...
        { icon: LinkedinIcon, link: "https://www.linkedin.com/in/fahad-cybersecurity-ai/" },
    ]
    
    const developerSocialIcons = [
        { icon: LinkedinIcon, link: "https://www.linkedin.com/in/fahad-cybersecurity-ai/", title: "LinkedIn" },
        { icon: GithubIcon, link: "https://github.com/cybe4sent1nel", title: "GitHub" },
    ]

    return (
        <footer className="mx-6 bg-white dark:bg-slate-900 transition">
            <div className="max-w-7xl mx-auto">
                <div className="flex flex-col md:flex-row items-start justify-between gap-10 py-10 border-b border-slate-500/30 dark:border-slate-700 text-slate-500 dark:text-slate-400">
                    <div>
                        <Link href="/" className="flex items-center gap-3 text-4xl font-semibold text-slate-700 dark:text-white">
                        ...
                        <p className="max-w-[410px] mt-6 text-sm">Welcome to CrashKart, your ultimate destination for the latest and smartest gadgets. From smartphones and smartwatches to essential accessories, we bring you the best in innovation — all in one place.</p>
                        <div className="flex items-center gap-3 mt-5">
                            {socialIcons.map((item, i) => (
                                <Link href={item.link} key={i} className="flex items-center justify-center w-10 h-10 bg-slate-100 dark:bg-slate-800 hover:scale-105 hover:border border-slate-300 dark:border-slate-600 transition rounded-full">
                                    <item.icon />
                                </Link>
                            ))}
                        </div>
                        ...
                        {linkSections.map((section, index) => (
                            <div key={index}>
                                <h3 className="font-medium text-slate-700 dark:text-white md:mb-5 mb-3">{section.title}</h3>
                ...
                <div className="py-6 flex flex-col sm:flex-row items-center justify-between gap-4 border-t border-slate-200 dark:border-slate-700 mt-4">
                    <p className="text-sm text-slate-500 dark:text-slate-400">
                        Copyright 2025 © CrashKart All Right Reserved.
                    </p>
                    <div className="flex items-center gap-4">
                        ...
                        <div className="flex items-center gap-2">
                            {developerSocialIcons.map((item, i) => (
                                <Link href={item.link} key={i} title={item.title} target="_blank" rel="noopener noreferrer" className="flex items-center justify-center w-8 h-8 bg-slate-200 dark:bg-slate-700 hover:bg-slate-300 dark:hover:bg-slate-600 hover:scale-110 transition rounded-full">
                                    <item.icon />
                                </Link>
                            ))}
                        </div>
