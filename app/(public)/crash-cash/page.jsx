...
import { getUserCrashCash, setUserCrashCash, migrateOldCrashCash } from '@/lib/userCrashcashUtils'
...
export default function CrashCashPage() {
    ...
    useEffect(() => {
         ...
         try {
             ...
             // Load crash cash for the specific user
             const userCashData = getUserCrashCash(email)
             const cashData = userCashData.items || []
             
             setCrashCashList(cashData)
             ...
             cashData.forEach(item => {
                 ...
                 if (expiryDate > new Date()) {
                     totalActive += item.amount
                 } else {
                     totalExpired += item.amount
                 }
             })
    ...
    const deleteCrashCash = (code) => {
         if (confirm('Are you sure you want to remove this Crash Cash?')) {
             ...
             try {
                 ...
                 const newBalance = totalActive
                 setUserCrashCash(email, newBalance, updated)
