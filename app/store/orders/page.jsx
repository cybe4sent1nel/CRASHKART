...
export default function StoreOrders() {
    ...
    const updateOrderStatus = async (orderId, status) => {
        ...
        // Save to localStorage (where user orders are stored)
        ...
        // Find the updated order to send full order data
        const updatedOrder = updatedOrders.find(o => o.id === orderId);

        // Notify user's orders page about the status update with full order data
        window.dispatchEvent(new CustomEvent('orderStatusUpdated', {
            detail: { 
                ...
                status,
                order: updatedOrder,
                updatedOrderFromAdmin: updatedOrder
            }
        }));

        console.log('✅ Order status updated:', { orderId, status, updatedOrder });
    }
                        ...
                        <div className="mb-4">
                            ...
                            <div className="space-y-2">
                                {selectedOrder.orderItems.map((item, i) => (
                                    <div key={i} className="flex items-center gap-4 border border-slate-100 shadow rounded p-3 hover:bg-gray-50 transition">
                                        <img
                                        ...
                                        {selectedOrder.status === 'DELIVERED' && (
                                            <div className="text-right">
                                                <p className="text-xs text-green-600 font-semibold mb-1 flex items-center gap-1"><Check size={14} /> Delivered</p>
                                                <p className="text-xs text-gray-500">Customer can review</p>
                                            </div>
                                        )}
                                    </div>
                                ))}
