...
export default function AdminReviewsPage() {
    ...
    const [activeTab, setActiveTab] = useState('product') // 'product' or 'app'
    ...
    const fetchReviews = async () => {
        try {
            console.log('🔍 Admin fetching product reviews')
            const response = await fetch('/api/ratings/get-all-ratings')
            const data = await response.json()
            console.log('⭐ Product reviews received:', data.ratings?.length || 0, 'items')
            if (data.success) {
    ...
    const fetchAppReviews = async () => {
        try {
            console.log('🔍 Admin fetching app reviews')
            const response = await fetch('/api/feedback?type=app&limit=100')
            const data = await response.json()
            console.log('📱 App reviews API response:', data)
            console.log('📱 App reviews received:', data.feedbacks?.length || 0, 'items')
            if (data.success) {
                setAppReviews(data.feedbacks || [])
            } else {
    ...
    const currentReviews = activeTab === 'product' ? reviews : appReviews
    const filteredReviews = filter === 'all' 
        ? currentReviews 
        : currentReviews.filter(r => r.rating === parseInt(filter))
    ...
    return (
        ...
        >
            ...
            <div className="mb-8">
                ...
                <p className="text-slate-600 dark:text-slate-400">All product reviews and app feedback</p>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-5 gap-6 mb-8">
                <div className="bg-white dark:bg-slate-800 rounded-xl p-6 border border-slate-200 dark:border-slate-700">
                    ...
                    <p className="text-3xl font-bold text-slate-800 dark:text-white">{currentReviews.length}</p>
                </div>
                <div className="bg-green-50 dark:bg-green-900/30 rounded-xl p-6 border border-green-200 dark:border-green-800">
                    ...
                    <p className="text-3xl font-bold text-green-800 dark:text-green-200">{currentReviews.filter(r => r.rating === 5).length}</p>
                </div>
                <div className="bg-blue-50 dark:bg-blue-900/30 rounded-xl p-6 border border-blue-200 dark:border-blue-800">
                    ...
                    <p className="text-3xl font-bold text-blue-800 dark:text-blue-200">{currentReviews.filter(r => r.rating === 4).length}</p>
                </div>
                <div className="bg-yellow-50 dark:bg-yellow-900/30 rounded-xl p-6 border border-yellow-200 dark:border-yellow-800">
                    ...
                    <p className="text-3xl font-bold text-yellow-800 dark:text-yellow-200">{currentReviews.filter(r => r.rating === 3).length}</p>
                </div>
                <div className="bg-red-50 dark:bg-red-900/30 rounded-xl p-6 border border-red-200 dark:border-red-800">
                    ...
                    <p className="text-3xl font-bold text-red-800 dark:text-red-200">{currentReviews.filter(r => r.rating <= 2).length}</p>
                </div>
            </div>
                                        ...
                                        <div className="flex items-start justify-between gap-4 mb-3">
                                            <div>
                                                <h3 className="text-lg font-bold text-slate-800 dark:text-white flex items-center gap-2">
                                                    ...
                                                    {review.title || 'App Review'}
                                                </h3>
                                                <p className="text-sm text-slate-600 dark:text-slate-400 mt-1">
                                                    By: {review.userName || 'Anonymous'} • {review.userEmail}
                                                </p>
                                                <p className="text-xs text-slate-500 dark:text-slate-500 mt-1">
                                                    Status: <span className={`font-medium ${review.status === 'approved' ? 'text-green-600' : 'text-yellow-600'}`}>
                                                        {review.status}
                                                    </span>
                                                </p>
                                            </div>
