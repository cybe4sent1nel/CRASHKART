...
export default function AdminLogin() {
     ...
     useEffect(() => {
          // Check if already verified - if so, redirect immediately
          const isVerified = localStorage.getItem('admin2FAVerified') === 'true' || sessionStorage.getItem('admin2FAVerified') === 'true'
          if (isVerified) {
              router.push('/admin')
              return
          }
          ...
          if (userEmail && userEmail === MAIN_ADMIN_EMAIL) {
              ...
              // Regular user logged in but trying to access admin - show email choice
              setProfileEmail(userEmail)
              setEmail('')
              setIsLoggedIn(false)
              setStep('choose')
